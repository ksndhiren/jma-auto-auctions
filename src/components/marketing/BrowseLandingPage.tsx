...
export interface BrowseLandingPageData {
  slug: string;
  pageTitle: string;
...
export function buildBrowseHead(page: BrowseLandingPageData) {
  return {
    meta: [
...
export function BrowseLandingPage({ page }: { page: BrowseLandingPageData }) {
  const faqGroups: FAQGroup[] = [
    { slug: page.slug, title: page.faqTitle, items: page.faqs },
  ];
    ...
    <SiteShell>
      ...
      <Section size="md">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
          <SectionHeader eyebrow={page.pageTitle} title={page.introTitle} description={page.introBody} />
          <div className="border border-border bg-bone p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">
              Category Snapshot
            </p>
            <div className="mt-4 space-y-3">
              {page.uses.slice(0, 4).map((use) => (
                <div key={use} className="border-l-2 border-gold pl-4 text-sm text-muted-foreground">
                  {use}
                </div>
              ))}
            </div>
      ...
      <Section tone="bone" size="md">
        <div className="grid gap-12 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <SectionHeader eyebrow="Why This Category" title={page.benefitsTitle} />
          </div>
      ...
      <Section size="md">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.2fr)]">
          <SectionHeader eyebrow="Before You Buy" title={page.considerationsTitle} />
      ...
      <Section tone="surface" size="md">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.2fr)]">
          <SectionHeader eyebrow="Use Cases" title={page.usesTitle} description={page.usesLead} />
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {page.uses.map((use) => (
              <div key={use} className="border border-border bg-background px-4 py-4 text-sm font-medium text-ink">
                {use}
              </div>
            ))}
          </div>
